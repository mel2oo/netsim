...
func NewServer(addr string, frwd *tunnel.Forwarder, config *Config) (*Server, error) {
	c, err := NewClient(config, frwd)
	if err != nil {
		return nil, err
	}
...
func (s *Server) ListenAndServeUDP(wg *sync.WaitGroup) {
	...
	wg.Done()
	if err != nil {
		logrus.Infof("[dns] failed to listen on %s, error: %v", s.addr, err)
		return
	}
	defer pc.Close()

	logrus.Infof("[dns] listening UDP on %s", s.addr)

	for {
		reqBytes := pool.GetBuffer(UDPMaxLen)
		n, caddr, err := pc.ReadFrom(reqBytes)
		if err != nil {
			logrus.Infof("[dns] local read error: %v", err)
...
func (s *Server) ServePacket(pc net.PacketConn, caddr net.Addr, reqBytes []byte) {
	...
	if err != nil {
		logrus.Infof("[dns] error in exchange for %s: %s", caddr, err)
		return
	}

	_, err = pc.WriteTo(respBytes, caddr)
	if err != nil {
		logrus.Infof("[dns] error in local write to %s: %s", caddr, err)
		return
	}
}
...
func (s *Server) ListenAndServeTCP(wg *sync.WaitGroup) {
	...
	wg.Done()
	if err != nil {
		logrus.Infof("[dns-tcp] error: %v", err)
		return
	}
	defer l.Close()

	logrus.Infof("[dns-tcp] listening TCP on %s", s.addr)

	for {
		c, err := l.Accept()
		if err != nil {
			logrus.Infof("[dns-tcp] error: failed to accept: %v", err)
			continue
		}
		go s.ServeTCP(c)
	}
...
func (s *Server) ServeTCP(c net.Conn) {
	...
	if err := binary.Read(c, binary.BigEndian, &reqLen); err != nil {
		logrus.Infof("[dns-tcp] failed to get request length: %v", err)
		return
	}
	...
	if err != nil {
		logrus.Infof("[dns-tcp] error in read reqBytes %s", err)
		return
	}
	...
	if err != nil {
		logrus.Infof("[dns-tcp] error in exchange: %s", err)
		return
	}
	...
	if _, err := (&net.Buffers{lenBuf, respBytes}).WriteTo(c); err != nil {
		logrus.Infof("[dns-tcp] error in write respBytes: %s", err)
	}
}
